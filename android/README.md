...
- Private, offline collection with optional Google sign-in through Firebase Authentication. No bank integration or analytics.
...
The private collection is stored at `files/collection.json` using `AtomicFile`. A mutex serializes mutations, and UI state is published only after a successful write. A malformed file is not silently replaced. Google sign-in sends account information to Google and Firebase. Subscription data is not sent to a server. Bundled assets work without connectivity.
...
5. Test on an Android device with current Google Play services and a Google account. Verify consent, cancellation, sign-out, restarting the app, and switching accounts. Authentication does not upload or isolate the existing device collection.
